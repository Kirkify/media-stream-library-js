...
describe('h264 handler', () => {
    ...
    it('parses a single NALU packet', () => {
        ...
        const remaining = h264depay(Buffer.alloc(0), { type: MessageType.RTP, data: singleNalu, channel: 0 }, callback);
    ...
    it('parses a FU-A frame split on two RTP packages', () => {
        ...
        /* eslint-enable */
        const remaining = h264depay(Buffer.alloc(0), { type: MessageType.RTP, data: fuaPart1, channel: 0 }, callback);
        expect(callback).toHaveBeenCalledTimes(0);
        expect(remaining.length).toBeGreaterThan(0);
        h264depay(remaining, { type: MessageType.RTP, data: fuaPart2, channel: 0 }, callback);
        expect(callback).toHaveBeenCalledTimes(1);
        const msg = callback.mock.calls[0][0];
